 ...
 * With a gaze (the cursor moved recently) it watches that direction instead of glancing.
 */
export function idlePose(t: number, blink: boolean, gaze: Gaze | null = null): Pose {
  const cycle = 3.7
  const n = Math.floor(t / cycle)
  const u = t - n * cycle
  if (blink && u > 3.56) return pose(front({ eyes: 'closed' }), { name: 'blink' })
  if (gaze) return pose(front({ gaze }), { name: `watch ${gaze.join(',')}` })
